...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 初始化隐藏状态
        batch_size = x.size(0)
        h_prev = torch.zeros(batch_size, self.gru.hidden_size).to(x.device)
        ...
        x = torch.cat(h_t, dim=1)  # 连接所有时间步的输出
        x = x.transpose(1, 2)  # 转置回 (batch_size, out_channels, sequence_length)
