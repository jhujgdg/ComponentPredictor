...
from model import TimesNetClassifierWrapper
...
configs = Configs()
base_model = Model(configs)

my_model = TimesNetClassifierWrapper(base_model, num_embeddings=num_embeddings)
...
# 将迭代数据保存为 DataFrame，并保存为 CSV 文件
df = pd.DataFrame(iteration_data)
df.to_csv('TimesNet_results.csv', index=False)
