...
def time_weighted_sum(w, x, eps=0.0):
    """
    纯PyTorch实现TimeX的功能：
    Args:
        w: (B, C, T) or (1, C, T) 权重张量
        x: (B, C, T) 要加权求和的张量
        eps: 防止除零
    Returns:
        output: (B, C, T) 加权输出
    ...
    for t in range(T):
        # w[..., :t+1] is shape (B, C, t+1), x[..., :t+1] same
        wt = w[..., :t+1]
        xt = x[..., :t+1]
        weighted_sum = (wt * xt).sum(dim=-1)
        weight_sum = wt.sum(dim=-1) + eps
        out[..., t] = weighted_sum / weight_sum
    return out
...
class RWKV_TimeMix(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.time_curve = self.time_curve.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    def forward(self, x):
        ...
        k = self.key(x).transpose(-1, -2)  # (B, C, T)
        v = self.value(x).transpose(-1, -2)  # (B, C, T)
        ...
        w = torch.exp(self.time_w)  # (1, C, T)

        wkv = time_weighted_sum(w.expand(B, -1, -1), kv)
        wk = time_weighted_sum(w.expand(B, -1, -1), k, eps=RWKV_K_EPS)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        c = c @ F.one_hot(idx.long(), num_classes=self.config.vocab_size).float()
        x = self.head(x) + c
        x = x[:, -1, :]
