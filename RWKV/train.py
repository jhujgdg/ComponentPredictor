...
class GPTConfig:
    def __init__(self,vocab_size,ctx_len,n_layer,n_embd,model_type):
...
for i in range(500):
    ...
    outputs,_ = my_model(train_input)
    ...
    my_model.eval()
    # 测试集评估
    test_outputs,_ = my_model(test_input)  # 获取模型输出
...
df.to_csv('RWKV_results.csv', index=False)
